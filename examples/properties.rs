...
fn base() {
    ...
    let mut instance = wasm.get_property(&context, "Instance".to_string()).to_object(&context);
...
fn base_with_exports() {
    ...
    let val = global.get_property(&context, "WebAssembly".to_string());
    let module_const = module.construct(&context, &[bytes.to_jsvalue()]).unwrap();
    let mut instance_const = instance.construct(&context, &[module_const.to_jsvalue()]).unwrap();
    println!("a");
    let mut exports = instance_const.get_property(&context, "exports".to_string()).to_object(&context);
    ...
    let result = sum.to_object(&context).call(&context, JSValue::undefined(&context).to_object(&context), &[JSValue::number(&context, 10 as _), JSValue::number(&context, 32 as _)]).unwrap();
...
fn base_with_imports() {
    ...
    let js_func = JSObject::new_function_with_callback(&context, "host_func1".to_string(), Some(foo));
    ...
    let mut instance_const = instance.construct(&context, &[module_const.to_jsvalue(), imports.to_jsvalue()]).unwrap();
    println!("a");
    let mut exports = instance_const.get_property(&context, "exports".to_string()).to_object(&context);
    println!("{:?}", exports.get_property_names(&context));
